...
class _BvnVerificationScreenState extends ConsumerState<BvnVerificationScreen> {
  ...
  Future<void> _handleContinue() async {
    ...
    try {
      ...
      // user doc (used by polling/audit). If the result can't be parsed, the
      // upload falls back to the legacy flat-path layout and we skip the
      ...
      // Verify the upload produced the storage paths required by the server-
      // side BIOMETRIC_KYC submission. If the SDK didn't capture liveness
      // frames or the upload didn't populate them, fail closed -- the
      // verification cannot succeed downstream without these.
      ...
      // Save SmileID userId and jobId for polling (preserved from prior flow).
      try {
                  ...
                  children: [
                    ...
                    KycVerificationCard(
                      ...
                      description: _isCaptured
                          ? 'Your BVN has been captured. Verification will begin when you continue.'
